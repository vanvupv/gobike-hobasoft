    ...
    class YITH_WooCommerce_Advanced_Reviews {
	    ...
	    public function get_reviews_stats( $product_id ) {
		    ...
		    if ( false === ( $review_stats = get_transient( $reviews_stats_transient_name ) ) ) {
			    ...
			    $review_stats['voted_reviews'] = $votedReviews;

			    if ( $votedReviews > 0 ) {
				    $average = $totalStars / $votedReviews;
			    }
			    else {
				    $average = 0;
			    }
        ...
        public function add_fields_to_comment_form( $comment_form ) {
            ...
            if ( $this->enable_attachments ) {
                $content_after = '<p class="upload_section ' . $hide_rating . '" >
					<label for="ywar-uploadFile" > ' . esc_html__( 'Thêm ảnh đánh giá', 'yith-woocommerce-advanced-reviews' ) . ' </label >
					<input type = "button" value = "' . esc_html__( 'Chọn ảnh', 'yith-woocommerce-advanced-reviews' ) . '" id = "do_uploadFile" />
					<input type = "file" name = "ywar-uploadFile[]" id = "ywar-uploadFile" multiple="multiple" />';

                $attachment_message = '';
                if ( trim( $this->attachment_allowed_type ) ) {
                    $attachment_message .= sprintf( esc_html__( "Các định dạng ảnh được chấp nhận: %s. ", 'yith-woocommerce-advanced-reviews' ), $this->attachment_allowed_type );
                }
