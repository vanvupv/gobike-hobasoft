...
class WOOF_ADV {
	...
	public function __construct( $alert_list = array() ) {
		...
		if ( isset( $_SERVER['REQUEST_URI'] ) ) {
			...
			if ( substr_count( sanitize_text_field( wp_unslash( $_SERVER['PHP_SELF'] ) ), 'theme-install.php' ) ) {
				return;
			}
		}
	...
	public function alert_woocommerce_currency_switcher() {
		...
		<?php // Inline script is required: contains a dynamic nonce value (wp_create_nonce) generated server-side for the AJAX dismiss request, and conditional PHP logic for the plugin-install screen. ?>
		<script>
			//DYNAMIC JAVASCRIPT
