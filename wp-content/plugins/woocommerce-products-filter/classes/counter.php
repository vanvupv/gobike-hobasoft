...
final class WP_QueryWoofCounter {
	...
	public function __construct( $query ) {
		...
		$this->table      = HUSKY::$query_cache_table;
		// ***
	...
	private function set_value() {
		...
		$wpdb->query(
			$wpdb->prepare(
				"INSERT INTO {$this->table} (mkey, mvalue) VALUES (%s, %d) ON DUPLICATE KEY UPDATE mvalue=%d",// phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared
				$this->key_string,
				$this->post_count,
				$this->post_count
			)
	...
	private function get_value() {
		...
		$value = $wpdb->get_results(
			$wpdb->prepare(
				"SELECT mkey, mvalue FROM {$this->table} WHERE mkey = %s",// phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared
				$this->key_string
			)
		);
