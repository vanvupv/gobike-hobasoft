...
final class WOOF_EXT_TURBO_MODE extends WOOF_EXT {
	...
	public function ajax_create_data_search_files() {
		if ( ! isset( $_REQUEST['turbo_nonce'] ) || ! wp_verify_nonce( sanitize_text_field( wp_unslash( $_REQUEST['turbo_nonce'] ) ), 'woof_turbo_nonce' ) ) {
			die( '0' );
		}
	...
	public function get_data_by_id( $id, $lang = 'xxx' ) {
		...
		if ( $product->get_type() == 'variation' ) {
			...
			foreach ( $var_attributes as $key_attr => $val ) {
				...
				$data['taxonomies'][ $key ][] = html_entity_decode( $val , ENT_QUOTES | ENT_HTML5, 'UTF-8' );
			}
		} elseif ( true ) {
			// if you want search in all taxonomies by additional filters( ignore $this->tax_serch_data )
			foreach ( $all_taxonomies as $key_slug => $val_tax ) {
				...
				foreach ( $_terms as $term ) {
					$data['taxonomies'][ $key_slug ][] = html_entity_decode( $term->slug , ENT_QUOTES | ENT_HTML5, 'UTF-8' );
				}
			}
		}
