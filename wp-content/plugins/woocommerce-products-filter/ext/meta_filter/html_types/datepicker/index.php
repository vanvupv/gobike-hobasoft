...
class WOOF_META_FILTER_DATEPICKER extends WOOF_META_FILTER_TYPE {
	...
	protected function draw_additional_options() {
		$this->render_html_e(
			$this->get_meta_filter_path() . '/views/additional_options.php',
			array(
				'key'      => $this->meta_key,
	...
	public function create_meta_query() {
		...
		if ( $curr_request ) {
			...
			if ( apply_filters( 'woof_meta_use_acf_dateformat', false ) ) {
				...
				$from = gmdate( 'Ymd', intval( $from ) ); // int must be another way fatal error because from DB it cames as string
				$to   = gmdate( 'Ymd', intval( $to ) ); // int must be another way fatal error because from DB it cames as string
			}
	...
	public static function get_option_name( $value, $key = null ) {
		...
		if ( count( $arr_val ) > 1 ) {
			...
			if ( $arr_val[0] and $arr_val[0] != 'i' ) {
				...
				$value_txt .= sprintf( esc_html__( 'from: %s', 'woocommerce-products-filter' ), gmdate( $format, $arr_val[0] ) );
			}
			if ( $arr_val[1] and $arr_val[1] != 'i' ) {
				$value_txt .= ' ';
				/* translators: %s: date value */
				$value_txt .= sprintf( esc_html__( 'to: %s', 'woocommerce-products-filter' ), gmdate( $format, $arr_val[1] ) );
			}
		}
