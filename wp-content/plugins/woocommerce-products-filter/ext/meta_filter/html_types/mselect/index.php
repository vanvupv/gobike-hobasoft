...
class WOOF_META_FILTER_MSELECT extends WOOF_META_FILTER_TYPE {
	...
	public static function get_option_name( $value, $key = null ) {
		...
		if ( $key ) {
			...
			$options_separator = apply_filters( 'woof_meta_options_separator', ',' );
			$options           = explode( $options_separator, ( isset( woof()->settings['meta_filter'][ $meta_key ]['options'] ) ) ? woof()->settings['meta_filter'][ $meta_key ]['options'] : '' );
			$value_arr         = explode( $options_separator, $value );
			$opt               = array();
			if ( ! empty( $options ) ) {
