...
class WOOF_META_FILTER_SLIDER extends WOOF_META_FILTER_TYPE {
	...
	public static function get_option_name( $value, $key = null ) {
		...
		if ( count( $arr_val ) > 1 ) {
			...
			/* translators: 1: currency prefix, 2: minimum value, 3: maximum value, 4: currency postfix */
			$value_txt = sprintf( __( 'from %1$s %2$s to %3$s %4$s', 'woocommerce-products-filter' ), $prefix, $arr_val[0], $arr_val[1], $postfix );
		}
		return $value_txt;
	}
}
