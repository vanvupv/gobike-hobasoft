...
if ( ! function_exists( 'woof_draw_label_childs' ) ) {
	...
	function woof_draw_label_childs( $taxonomy_info, $tax_slug, $childs, $show_count, $show_count_dynamic, $hide_dynamic_empty_pos, $term_id = 0 ) {
				...
				foreach ( $childs as $term ) :
					...
					if ( woof()->settings['dispay_in_row'][ $tax_slug ] and empty( $term['childs'] ) ) :
						...
						if ( ! $count and ! in_array( $term['slug'], $current_request ) and $show_count ) :
							?>
							disabled=""<?php endif; ?> id="<?php echo esc_attr( 'woof_' . $term['term_id'] . '_' . $inique_id ); ?>" class="woof_label_term" data-tax="<?php echo esc_attr( woof()->check_slug( $tax_slug ) ); ?>" name="<?php echo esc_html( $term['slug'] ); ?>" value="<?php echo esc_html( $term['term_id'] ); ?>" <?php checked( in_array( $term['slug'], $current_request ) ); ?> />&nbsp;<label for="<?php echo esc_attr( 'woof_' . $term['term_id'] . '_' . $inique_id ); ?>" 
							<?php
	...
	<?php if ( ! empty( $terms ) ) : ?>
		...
		foreach ( $terms as $term ) :
			...
			if ( $hide_next_term_li ) :
				?>
				woof_hidden_term<?php endif; ?>" style="
				<?php
				if ( $this->settings['dispay_in_row'][ $tax_slug ] and empty( $term['childs'] ) ) :
					?>
				display: inline-block !important;<?php endif; ?>">
				...
				<span class="woof_label_term <?php echo( boolval( $checked ) ? 'checked' : '' ); ?>">
					...
					<input id="<?php echo esc_attr( $label_id ); ?>" style="display: none;" type="checkbox" 
					<?php
					if ( ! $count and ! in_array( $term_slug, $current_request ) and $show_count ) :
						?>
