			...
			if ( count( $from_to ) == 2 ) {
					...
					$product_variations = $wpdb->get_results(
						$wpdb->prepare(
							"
                                        ...
                                        WHERE posts.post_type IN ('product','product_variation')
                                        AND postmeta.meta_key = '_stock'
                                        AND postmeta.meta_value >= %d AND postmeta.meta_value < %d",
							$stock_quantity_from,
							$stock_quantity_to
						),
						ARRAY_N
				...
				if ( ! empty( $product_variations ) ) {
					...
					$products = $wpdb->get_results(
                        ...
                        FROM {$wpdb->posts} AS posts
                        WHERE posts.ID IN ($product_variations_ids_string) AND posts.post_parent > 0",// phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared
						ARRAY_N
					);
					if ( ! empty( $products ) ) {
