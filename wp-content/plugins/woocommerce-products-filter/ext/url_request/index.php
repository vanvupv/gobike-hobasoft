...
final class WOOF_EXT_URL_REQUEST extends WOOF_EXT {
	...
	public function init_js() {
		...
		if ( version_compare( $wp_version, '6.3', '>=' ) ) {
			wp_enqueue_script(
				'woof_url_parser',
				$this->get_ext_link() . 'js/url_parser.js',
				array( 'jquery' ),
				WOOF_VERSION,
				array(
					'in_footer' => true,
	...
	public function get_seo_rule_html() {
		if ( ! isset( $_REQUEST['url_request_nonce'] ) || ! wp_verify_nonce( sanitize_text_field( wp_unslash( $_REQUEST['url_request_nonce'] ) ), 'woof_url_request_nonce' ) ) {
			die( '0' );
		}
	...
	public function woof_print_applications_tabs_content() {
		wp_enqueue_script( 'woof_seo_admin', $this->get_ext_link() . 'js/admin.js', array(), WOOF_VERSION );
		wp_enqueue_style( 'woof_seo_admin_css', $this->get_ext_link() . 'css/admin.css', array(), WOOF_VERSION );
		// ***
