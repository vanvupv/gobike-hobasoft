...
require_once 'classes/additional.php';
require_once 'classes/options.php';
require_once 'classes/options-layout.php';
require_once 'classes/options-sections.php';
require_once 'classes/options-sections-layout.php';
require_once 'classes/viewtypes.php';
...
final class WOOF_FRONT_BUILDER extends WOOF_EXT {
	...
	public function __construct() {
		...
		add_filter( 'woof_modify_settings_before_action', array( $this, 'woof_modify_settings_before_action' ), 10, 2 );
		add_filter( 'woof_get_request_data', array( $this, 'woof_get_request_data' ) );
		add_filter( 'woof_get_meta_query', array( $this, 'woof_get_meta_query' ), 1 );
		add_filter( 'woof_regulate_by_only_show', array( $this, 'woof_regulate_by_only_show' ), 10, 2 );
	...
	public function woof_modify_settings_before_action( $settings, $atts ) {
		...
		if ( isset( $name ) ) {
			...
			$section_options['tax_block_height'] = array();
	...
	private function init_request() {
		$plugin_options = get_option( 'woof_settings', array() );
		...
		if ( wp_doing_ajax() ) {
			...
			if ( isset( $_REQUEST['woof_shortcode'] ) ) {
				...
				$atts = shortcode_parse_atts( stripcslashes( $_REQUEST['woof_shortcode'] ) );// phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized
	...
	public function woof_get_request_data( $data ) {
		...
		if ( $filter_id > 0 ) {
			woof()->settings = $this->woof_modify_settings_before_action(
				woof()->settings,
				array(
					'name'      => $this->get_name_by_id( $filter_id ),
					'filter_id' => $filter_id,
				)
			);
		}
	...
	public function wp_footer() {
		...
		wp_enqueue_style( 'woof-front-builder-css', $this->get_ext_link() . 'css/front-builder.css', array(), WOOF_VERSION );

		if ( ! $this->is_admin() ) {
			return false;
		}

		wp_enqueue_style( 'woof-front-builder-popup-23', $this->get_ext_link() . 'css/popup-23.css', array(), WOOF_VERSION );

		wp_enqueue_style( 'woof-front-builder-table', $this->get_ext_link() . 'css/table.css', array(), WOOF_VERSION );

		wp_enqueue_script( 'woof-front-builder-boot', $this->get_ext_link() . 'js/boot.js', array(), WOOF_VERSION );
		wp_enqueue_script( 'woof-front-builder', $this->get_ext_link() . 'js/front-builder.js', array(), WOOF_VERSION );
	...
	public function admin_footer() {
		if ( isset( $_GET['woof-action'] ) and $_GET['woof-action'] === 'form-builder' ) {
			...
			?>
			<?php // Inline style is intentional: this CSS hides WordPress admin chrome when the Form Builder runs inside an iframe. One value (max_height) is dynamic and cannot be moved to a static file; it is sanitized via intval(). ?>
			<style>
				#wpbody-content,
				#wpcontent,
				#wpbody
			...
			<?php // Inline script is required: contains a dynamic value (sd_id) passed from PHP to JS for the Smart Designer interface. The value is sanitized via intval(). ?>
			<script>
	...
	private function get_data( $name ) {
		...
		$data['selected_taxonomies']    = array();
		$data['selected_nontaxonomies'] = array();

		if ( empty( $data['selected'] ) ) {
			$data['selected'] = array(); // NULL
		} else {
	...
	public function get_items() {
		...
		$viewtypes = array_merge( woof()->settings['tax_type'], $this->viewtypes->get( sanitize_text_field( wp_unslash( $_REQUEST['name'] ) ) ) );
	...
	public function save_items() {
		...
		$fields = sanitize_text_field( wp_unslash( $_REQUEST['fields'] ) );
