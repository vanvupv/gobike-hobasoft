	...
	public function __construct() {
		...
		add_action(
			...
			function () {
				...
				echo json_encode( $this->gets( sanitize_key( wp_unslash( $_REQUEST['type'] ) ) ) );
				exit;
			}
		);

		add_action(
			'wp_ajax_woof_sd_create_preset',
			function () {
				if ( ! isset( $_REQUEST['sd_nonce'] ) || ! wp_verify_nonce( sanitize_text_field( wp_unslash( $_REQUEST['sd_nonce'] ) ), 'woof_sd_nonce' ) ) {
					die( '0' );
				}
				...
				wp_send_json( strval( $this->create( sanitize_text_field( wp_unslash( $_REQUEST['title'] ) ), sanitize_key( wp_unslash( $_REQUEST['type'] ) ), intval( $_REQUEST['element_id'] ) ) ) );
			}
		);

		add_action(
			'wp_ajax_woof_sd_apply_preset',
			function () {
				if ( ! isset( $_REQUEST['sd_nonce'] ) || ! wp_verify_nonce( sanitize_text_field( wp_unslash( $_REQUEST['sd_nonce'] ) ), 'woof_sd_nonce' ) ) {
					die( '0' );
				}
		...
		add_action(
			...
			function () {
				...
				$this->update( intval( $_REQUEST['option_id'] ), sanitize_text_field( wp_unslash( $_REQUEST['value'] ) ) );
				die( '1' );
			}
		);

		add_action(
			'wp_ajax_woof_sd_delete_preset',
			function () {
				if ( ! isset( $_REQUEST['sd_nonce'] ) || ! wp_verify_nonce( sanitize_text_field( wp_unslash( $_REQUEST['sd_nonce'] ) ), 'woof_sd_nonce' ) ) {
					die( '0' );
				}
		...
		add_action(
			'woof_print_applications_tabs_content_smart_designer',
			function () {
