...
final class WOOF_SD_COLOR {
	...
	public function __construct() {
		add_action(
			'wp_ajax_woof_sd_load_color_terms',
			function () {
				...
				echo json_encode( $this->gets( sanitize_key( wp_unslash( $_REQUEST['taxonomy'] ) ), true, $term_id ) );
				exit;
			}
		);

		add_action(
			'wp_ajax_woof_sd_change_term_color',
			function () {
				if ( ! isset( $_REQUEST['sd_nonce'] ) || ! wp_verify_nonce( sanitize_text_field( wp_unslash( $_REQUEST['sd_nonce'] ) ), 'woof_sd_nonce' ) ) {
					die( '0' );
				}
				...
				$this->set( intval( $_REQUEST['term_id'] ), sanitize_text_field( wp_unslash( $_REQUEST['value'] ) ) );
				die( json_encode( array() ) );
			}
		);

		add_action(
			'wp_ajax_woof_sd_change_term_color_image',
			function () {
				if ( ! isset( $_REQUEST['sd_nonce'] ) || ! wp_verify_nonce( sanitize_text_field( wp_unslash( $_REQUEST['sd_nonce'] ) ), 'woof_sd_nonce' ) ) {
					die( '0' );
				}
				...
				$this->set_image( intval( $_REQUEST['term_id'] ), sanitize_text_field( wp_unslash( $_REQUEST['value'] ) ) );
				die( json_encode( array() ) );
			}
		);

		add_filter(
			'get_woof_sd_term_color',
			function ( $term_id ) {
	...
	private function gets( $tax_slug, $use_subterms = true, $parent_term_id = 0 ) {
		...
		if ( ! empty( $terms ) ) {
			foreach ( $terms as $term_id => $term ) {
				...
				$title = array(
					'value'       => $term['title'],
					'css_classes' => array( 'woof-sd-flex' ),
				);
				if ( $term['has_childs'] ) {
					$title = array(
						'value'       => array(
							...
							'value'   => $term['title'],
							'data-id' => $term_id,
							'action'  => 'woof_sd_childs_term_color',
						),
						'css_classes' => array( 'woof-sd-flex' ),
					);
				}

				$rows[ (string) $term_id ] = array(
					'title' => $title,
					'color' => array(
						'value'       => array(
							'element' => 'color',
							'value'   => $term['color'],
							'action'  => 'woof_sd_change_term_color',
						),
						'css_classes' => array( 'woof-sd-flex' ),
					),
					'image' => array(
						'value' => array(
							'element' => 'image',
							'value'   => $term['image'],
							'action'  => 'woof_sd_change_term_color_image',
						),
					),
				);
			}
		...
		$data = array(
			'header' => array(
				...
				array(
					'value' => esc_html__( 'Color', 'woocommerce-products-filter' ),
					'width' => '35%',
					'key'   => 'color',
				),
				array(
					'value' => esc_html__( 'Image', 'woocommerce-products-filter' ),
					'width' => '35%',
					'key'   => 'image',
				),
			),
