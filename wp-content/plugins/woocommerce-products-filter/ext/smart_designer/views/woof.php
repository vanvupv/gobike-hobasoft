...
if (!function_exists('woof_sd_assemble_terms')) {
	...
	function woof_sd_assemble_terms(
		$tax_slug,
		$terms,
		$hidden_terms,
		$hide_childs,
		...
		foreach ($terms as $term) {
			...
			$html = str_replace('__CONTENT__', has_filter('woof_before_term_name') ? apply_filters('woof_before_term_name', $term, (object) array()) : $term['name'], $html);
			...
			if (strpos($html, '__RESET_RADIO_BTN__') !== false) {
				$html = str_replace(
					'__RESET_RADIO_BTN__',
					WOOF_HELPER::generate_html_item(
						'a',
						array(
							'href' => '#',
							'rel' => 'nofollow',
...
<div class="woof_list woof_list_sd woof_list_<?php echo esc_attr($sd_type); ?>_sd woof_list_<?php echo esc_attr($sd_type); ?>_sd_<?php echo esc_attr($sd_template_num); ?>" style="<?php echo esc_attr(trim($sd_style)); ?>">
	...
	if (!empty($terms) and is_array($terms)) {
		// phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- HTML is assembled from internal plugin template files which are not user-accessible. Dynamic values (term IDs, slugs, taxonomy names) are escaped via esc_attr() inline. wp_kses() cannot be used as it strips onchange/onclick handlers required for filter interactivity.
