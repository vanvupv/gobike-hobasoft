...
final class WOOF_EXT_STAT extends WOOF_EXT {
	...
	public function woof_stat_check_connection() {
		if ( ! isset( $_REQUEST['stat_nonce'] ) || ! wp_verify_nonce( sanitize_text_field( wp_unslash( $_REQUEST['stat_nonce'] ) ), 'woof_stat_nonce' ) ) {
			die( '0' );
		}
		$pdo_options                 = array();
		...
		try {
            // phpcs:disable WordPress.DB.RestrictedClasses.mysql__PDO -- PDO required for external statistics DB connection. $wpdb cannot connect to external databases.
	...
	public function woof_stat_update_db() {
		...
		$row = $wpdb->get_results(
			$wpdb->prepare(
				"SELECT COLUMN_NAME FROM INFORMATION_SCHEMA.COLUMNS WHERE table_name = %s AND column_name = 'meta_value'",
				$this->table_stat_buffer
			)
		);

		if ( empty( $row ) ) {
            // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared -- Table name is an internal class property, not user input.
			$result = $wpdb->query( 'ALTER TABLE ' . $this->table_stat_buffer . ' ADD meta_value text COLLATE utf8_unicode_ci NOT NULL' );
			if ( $result === false ) {
				die( esc_html__( 'Something wrong!', 'woocommerce-products-filter' ) );
			}
		}
		update_option( 'woof_stat_updated_table', true );
	...
	public function woof_print_applications_tabs_content() {
		...
		// External service: Google Charts API is required for statistics visualization.
		// Google Charts cannot be self-hosted as per Google's terms of service.
		// See: https://developers.google.com/chart/interactive/docs/basic_load_libs
        // phpcs:ignore WordPress.WP.EnqueuedResources.NonEnqueuedScript, PluginCheck.CodeAnalysis.EnqueuedResourceOffloading.OffloadedContent -- Google Charts API cannot be self-hosted per Google's terms of service. See: https://developers.google.com/chart/interactive/docs/basic_load_libs
		...
		if ( isset( $data['stat_min_date'][0] ) ) {
			...
			$localize_script['min_year']  = gmdate( 'Y' );
			$localize_script['min_month'] = gmdate( 'n' );
		}
		...
		if ( version_compare( $memory, $min_memory_mb, '<' ) ) {
			...
			<div class="error">
				<p class="description">
					<?php
					/* translators: 1: minimum recommended memory size, 2: current memory size */
					printf( esc_html__( 'Very recommend for the statistic not less than %1$s of the memory to avoid malfunctionality. Now is on the site %2$s', 'woocommerce-products-filter' ), esc_html( size_format( $min_memory_mb ) ), esc_html( size_format( $memory ) ) );
					?>
				</p>
			</div>
			<?php
	...
	public function init_pdo() {
		// PDO is used intentionally here to connect to a separate,
		// user-configured statistics database server (not the WordPress database).
		// $wpdb cannot support external database connections with custom credentials.
		// This is a deliberate feature of the Statistics extension.
	...
	public function woof_write_stat() {
		...
		if ( ! isset( $_REQUEST['nonce_filter'] ) || ! wp_verify_nonce( sanitize_text_field( wp_unslash( $_REQUEST['nonce_filter'] ) ), 'woof_front_nonce' ) ) {
			die( '0' );
		}
	...
	public function get_operative_tables() {
		...
		if ( current_user_can( 'create_users' ) ) {
			...
			$calendar_from = mktime( 0, 0, 0, gmdate( 'n', $calendar_from ), gmdate( 'd', $calendar_from ), gmdate( 'y', $calendar_from ) );
			$calendar_to   = intval( WOOF_REQUEST::get( 'calendar_to' ) );
			$calendar_to   = mktime( 23, 59, 59, gmdate( 'n', $calendar_to ), gmdate( 'd', $calendar_to ), gmdate( 'y', $calendar_to ) );

			// +++

			$tables         = $this->get_stat_tables();
			$request_tables = array();
			$start_year     = gmdate( 'Y', $calendar_from );
			$start_month    = gmdate( 'n', $calendar_from );
			$finish_year    = gmdate( 'Y', $calendar_to );
			$finish_month   = gmdate( 'n', $calendar_to );
			// ***
	...
	public function woof_get_stat_data() {
		...
		if ( current_user_can( 'create_users' ) ) {
			...
			if ( ! is_null( $this->pdo ) ) {
				...
				// +++
                // phpcs:disable WordPress.DB.RestrictedClasses.mysql__PDO -- PDO required for external statistics DB. $wpdb cannot connect to external databases.
	...
	public function woof_get_request_data( $data ) {
		...
		if ( $do ) {
			...
			static $getting_term = false;//!! important fix to avoid function recursion
			if ( ! $getting_term ) {
				$getting_term = true;
			...
			if ( ! empty( $request ) and ( $this->get_user_requests_count( $user_ip ) < $this->user_max_requests ) ) {
				...
				// ***
				$hash = md5( json_encode( $request ) . $user_ip . gmdate( 'd-m-Y' ) );
				unset( $request['really_curr_tax'] );
				// lets check for the same request from the same user
				$the_same_hash = $wpdb->get_var(
					$wpdb->prepare(
						"SELECT hash FROM {$this->table_stat_tmp} WHERE user_ip = %s AND hash = %s",// phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared
						$user_ip,
						$hash
					)
				);
				...
				if ( empty( $the_same_hash ) ) {
					...
					$wpdb->query(
						$wpdb->prepare(
							"INSERT INTO {$this->table_stat_tmp} (user_ip, page, request, hash, tax_page, tax_page_term_id, time) VALUES (%s, %s, %s, %s, %s, %d, %d)",// phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared
							$user_ip,
							$type,
							$request,
							$hash,
	...
	public function assemble_stat() {
		...
		$count    = $wpdb->get_var(
			$wpdb->prepare(
				"SELECT COUNT(*) FROM {$this->table_stat_tmp} WHERE is_collected = %d ORDER BY user_ip",// phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared
				0
			)
		);
		...
		while ( true ) {
			...
			$res  = $wpdb->get_results(
				$wpdb->prepare(
					"SELECT * FROM {$this->table_stat_tmp} WHERE is_collected = %d LIMIT %d, %d",// phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared
					0,
					$step_num,
					$next
				),
			...
			if ( ! empty( $res ) ) {
				foreach ( $res as $row ) {
					...
					if ( ! empty( $data ) ) {
						foreach ( $data as $taxonomy => $term_slugs ) {
							...
							foreach ( $terms as $term_slug ) {
								...
								} elseif ( in_array( $taxonomy, $meta_key ) ) {
								...
								$wpdb->query(
									$wpdb->prepare(
										"INSERT INTO {$this->table_stat_buffer} (hash, user_ip, taxonomy, value, meta_value, page, tax_page_term_id, time) VALUES (%s, %s, %s, %d, %s, %s, %d, %d)",// phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared
										$row['hash'],
										$row['user_ip'],
										$taxonomy,
										$value,
					...
					$wpdb->query(
						$wpdb->prepare(
							"UPDATE {$this->table_stat_tmp} SET is_collected = %d WHERE hash = %s",// phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared
							1,
							$row['hash']
						)
					);
		...
		$wpdb->query(
			$wpdb->prepare(
				"DELETE FROM {$this->table_stat_tmp} WHERE is_collected = %d",// phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared
				1
			)
		);
		...
		if ( $this->place_statdata_into_db() ) {// placing data into dedicated DB
			...
			$wpdb->query( "TRUNCATE TABLE {$this->table_stat_buffer}" );// phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared
		}
		// ***
	...
	public function place_statdata_into_db() {
		...
		try {
			...
			$res = $wpdb->get_results( "SELECT DISTINCT hash FROM {$this->table_stat_buffer}", ARRAY_N );// phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared
			...
			if ( ! empty( $res ) ) {
				...
				foreach ( $hash_array as $hash ) {
					$res = $wpdb->get_results(
						$wpdb->prepare(
							"SELECT * FROM {$this->table_stat_buffer} WHERE hash = %s",// phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared
							$hash
						),
						ARRAY_A
					);
					...
					if ( ! empty( $res ) ) {
						...
						// PDO here
						if ( ! is_null( $this->pdo ) ) {
							$table = gmdate( 'Y', $time ) . '_' . gmdate( 'n', $time );
	...
	public function get_stat_tables() {
		if ( $this->pdo ) {
            // phpcs:disable WordPress.DB.RestrictedClasses.mysql__PDO -- PDO required for external statistics DB.
		...
		try {
			...
			if ( ! empty( $res ) ) {
				...
				foreach ( $hash_array as $hash ) {
					...
					if ( ! empty( $res ) ) {
						$time = $res[0]['time'];
						if ( ! isset( $results[ gmdate( 'Y', $time ) ] ) ) {
							$results[ gmdate( 'Y', $time ) ] = array();
						}
						if ( ! isset( $results[ gmdate( 'Y', $time ) ][ gmdate( 'n', $time ) ] ) ) {
							$results[ gmdate( 'Y', $time ) ][ gmdate( 'n', $time ) ] = array();
						}
						if ( ! isset( $results[ gmdate( 'Y', $time ) ][ gmdate( 'n', $time ) ][ gmdate( 'd', $time ) ] ) ) {
							$results[ gmdate( 'Y', $time ) ][ gmdate( 'n', $time ) ][ gmdate( 'd', $time ) ] = array();
						}
						...
						$results[ gmdate( 'Y', $time ) ][ gmdate( 'n', $time ) ][ gmdate( 'd', $time ) ][] = $res;
					}
				}
				// ***
				if ( ! empty( $results ) ) {
	...
	private function push_data_into_file( $data, $year, $month, $day ) {
		...
		if ( $wp_filesystem->put_contents( $file, json_encode( $data ), FS_CHMOD_FILE ) ) {
			return true;
		}
	...
	private function create_stat_file( $year, $month, $day ) {
		...
		WOOF_REQUEST::set( 'woof_stat_errors', array() );
		if ( ! $wp_filesystem->is_dir( $yf ) ) {
			if ( ! $wp_filesystem->mkdir( $yf, FS_CHMOD_DIR ) ) {
				/* translators: %s: folder path */
				WOOF_REQUEST::push( 'woof_stat_errors', sprintf( __( 'Its not possible to create folder: %s', 'woocommerce-products-filter' ), $yf ) );
				return false;
			}
		}
		if ( ! $wp_filesystem->is_dir( $mf ) ) {
			if ( ! $wp_filesystem->mkdir( $mf, FS_CHMOD_DIR ) ) {
				/* translators: %s: folder path */
	...
	public function get_stat_files_structure() {
		...
		if ( ! is_dir( $this->cache_folder ) ) {
			if ( ! $wp_filesystem->mkdir( $this->cache_folder ) ) {
				return array();
			}
		}
	...
	private function get_user_requests_count( $user_ip ) {
		...
		return (int) $wpdb->get_var(
			$wpdb->prepare(
				"SELECT COUNT(*) as count FROM {$this->table_stat_tmp} WHERE user_ip = %s",// phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared
				$user_ip
			)
		);
	}
