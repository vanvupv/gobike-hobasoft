...
final class WOOF_EXP_IMP extends WOOF_EXT {
	...
	public function get_export_data() {
		...
		if ( ! wp_verify_nonce( sanitize_text_field( wp_unslash( $_REQUEST['_nonce'] ) ), 'woof_export_settings' ) ) {
	...
	public function do_import_data() {
		...
		if ( ! wp_verify_nonce( sanitize_text_field( wp_unslash( $_REQUEST['_nonce'] ) ), 'woof_import_settings' ) ) {
			die( esc_html__( 'Error! Security issue', 'woocommerce-products-filter' ) );
		}

		try {
			$options = wc_clean( json_decode( stripcslashes( $_POST['import_value'] ), true ) );// phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized
