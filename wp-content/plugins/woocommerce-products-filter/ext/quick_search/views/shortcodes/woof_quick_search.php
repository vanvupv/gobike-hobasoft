		...
		if ( $extended_filter ) {
			...
			if ( $add_filters !== '' ) {
				...
				foreach ( $filter_items as $item ) {
					...
					switch ( $filter_struct[0] ) {
						...
						case 'slider':
							?>
							<p class="woof_notice"><a href="#"><?php esc_html_e( 'Please read documents', 'woocommerce-products-filter' ); ?></a></p>
							<?php
							break;
						default:
							break;
			...
			if ( $reset_btn == 1 ) {
				...
				<div class="woof_qt_reset_filter_con">
					<button class="woof_qt_reset_filter_btn"><?php echo esc_html( $reset_text ); ?></button>
				</div>
				<?php
			}
		}
		...
		switch ( $type ) {
			case 'drop-down':
				...
				if ( ! isset( $meta_item['options'] ) or ! $meta_item['options'] ) {
					?>
					<div class="woof_qt_add_filter "><p class="woof_notice"><a href="#"><?php esc_html_e( 'Error! Please read documents', 'woocommerce-products-filter' ); ?></a></p></div>
					<?php
					break;
				} else {
