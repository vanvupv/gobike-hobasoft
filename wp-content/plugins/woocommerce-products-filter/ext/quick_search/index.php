...
final class WOOF_EXT_QUICK_TEXT extends WOOF_EXT {
	...
	public function quick_search( $atts ) {
		...
		if ( file_exists( $this->get_ext_override_path() . 'views' . DIRECTORY_SEPARATOR . 'shortcodes' . DIRECTORY_SEPARATOR . 'woof_quick_search.php' ) ) {
			return woof()->render_html( $this->get_ext_override_path() . 'views' . DIRECTORY_SEPARATOR . 'shortcodes' . DIRECTORY_SEPARATOR . 'woof_quick_search.php', $data );// nosemgrep
		}
		return woof()->render_html( $this->get_ext_path() . 'views' . DIRECTORY_SEPARATOR . 'shortcodes' . DIRECTORY_SEPARATOR . 'woof_quick_search.php', $data );// nosemgrep
	}
	...
	public function quick_search_result( $atts ) {
		...
		if ( file_exists( $this->get_ext_override_path() . 'views' . DIRECTORY_SEPARATOR . 'shortcodes' . DIRECTORY_SEPARATOR . 'woof_quick_search_results.php' ) ) {
			return woof()->render_html( $this->get_ext_override_path() . 'views' . DIRECTORY_SEPARATOR . 'shortcodes' . DIRECTORY_SEPARATOR . 'woof_quick_search_results.php', $all_data );// nosemgrep
		}
		return woof()->render_html( $this->get_ext_path() . 'views' . DIRECTORY_SEPARATOR . 'shortcodes' . DIRECTORY_SEPARATOR . 'woof_quick_search_results.php', $all_data );// nosemgrep
	}
	...
	public function woof_print_applications_tabs_content() {
		...
		wp_enqueue_style( 'woof_qs_admin_css', $this->get_ext_link() . 'css/admin.css', array(), WOOF_VERSION );
	...
	public function create_data_search_files() {
        ...
        // phpcs:ignore WordPress.Security.NonceVerification.Missing -- Nonce is verified in ajax_create_data_search_files() before this method is called.
	...
	private function rename_files( $id ) {
		...
		$dir_path = $this->get_ext_path() . '/data/';
		$langs    = $this->get_all_lang_wpml();

		if ( $langs ) {
			foreach ( $langs as $lang ) {
				$src = $dir_path . 'temp/' . 'data_' . $id . '_' . $lang['language_code'] . '.json';
				$dst = $dir_path . 'data_' . $id . '_' . $lang['language_code'] . '.json';
				if ( $wp_filesystem->exists( $src ) ) {
					$wp_filesystem->move( $src, $dst, true );
				}
			}
		} else {
			$src = $dir_path . 'temp/' . 'data_' . $id . '_xxx.json';
			$dst = $dir_path . 'data_' . $id . '_xxx.json';
			if ( $wp_filesystem->exists( $src ) ) {
				$wp_filesystem->move( $src, $dst, true );
			}
		}
	...
	public function do_recreate_file( $id, $ids, $is_end ) {
		...
		if ( count( $ids ) < 1 or $is_end ) {
			...
			if ( $langs ) {
				foreach ( $langs as $lang ) {
					$src = $dir_path . 'data_' . $id . '_temp_' . $lang['language_code'] . '.json';
					$dst = $dir_path . 'data_' . $id . '_' . $lang['language_code'] . '.json';
					if ( $wp_filesystem->exists( $src ) ) {
						$wp_filesystem->move( $src, $dst, true );
					}
				...
				$src = $dir_path . 'data_' . $id . '_temp_xxx.json';
				$dst = $dir_path . 'data_' . $id . '_xxx.json';
				if ( $wp_filesystem->exists( $src ) ) {
					$wp_filesystem->move( $src, $dst, true );
				}
	...
	public static function show_sort_html_select() {
		$data = array(
			/* translators: %s: arrow symbol indicating sort direction */
