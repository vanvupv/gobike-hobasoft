...
final class WOOF_EXT_BY_TEXT extends WOOF_EXT {
	...
	public function wp_head() {
		...
		wp_enqueue_script( 'woof-husky', $this->get_ext_link() . 'assets/js/husky.js', array(), WOOF_VERSION );
	...
	private function data_fields( $args = array() ) {
		$data = array();
	...
	private function render_html( $pagepath, $data = array() ) {
	...
	private function render_html_e( $pagepath, $data = array() ) {
		if ( isset( $data['pagepath'] ) ) {
			unset( $data['pagepath'] );
		}
		if ( is_array( $data ) and ! empty( $data ) ) {
	...
	public function woof_text_filter( $args = array() ) {
		...
		if ( file_exists( $this->get_ext_override_path() . 'views' . DIRECTORY_SEPARATOR . 'shortcodes' . DIRECTORY_SEPARATOR . 'woof_text_filter.php' ) ) {
			return woof()->render_html( $this->get_ext_override_path() . 'views' . DIRECTORY_SEPARATOR . 'shortcodes' . DIRECTORY_SEPARATOR . 'woof_text_filter.php', array( 'data' => $this->data_fields( $args ) ) );
		}

		return woof()->render_html( $this->get_ext_path() . 'views' . DIRECTORY_SEPARATOR . 'shortcodes' . DIRECTORY_SEPARATOR . 'woof_text_filter.php', array( 'data' => $this->data_fields( $args ) ) );
	}
	...
	public function create_where_query( $search_terms ) {
		...
		if ( ! is_array( $search_terms ) ) {
			$search_terms = htmlspecialchars_decode( $search_terms , ENT_QUOTES | ENT_HTML5 );
		...
		foreach ( $search_terms as $term ) {
			...
			foreach ( $general_search_terms as $terms ) {
				$sub_sql[] = $wpdb->prepare( "({$wpdb->posts}.{$terms} $search_type %s)", $like );// phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared
			}
			if ( $tax_search ) {
				$sub_sql[] = $wpdb->prepare( "( trm.name $search_type %s)", implode( ' ', $search_terms ) );// phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared
			}
			if ( $meta_search && ! empty( $meta_search_terms ) ) {
				foreach ( $meta_search_terms as $meta_term ) {
					$sub_sql[] = $wpdb->prepare( "( postmeta.meta_key = %s AND  postmeta.meta_value $search_type %s)", $meta_term, $like );// phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared
				}
			}
		...
		if ( 1 == $search_on_vardesc ) {
			...
			if ( ! $product_ids ) {
				...
				foreach ( $search_terms as $term ) {
					...
					$sub_sql_var[] = $wpdb->prepare( " (postmeta.meta_value $search_type %s )", $like );// phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared
				}
				$condtion_string = implode( ' AND ', $sub_sql_var );
                // phpcs:disable WordPress.DB.PreparedSQL.NotPrepared -- $condtion_string consists of fragments already prepared via $wpdb->prepare() in the loop above.
				$product_variations = $wpdb->get_results(
					"
							...
							AND ($condtion_string)",// phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared
					ARRAY_N
				);
				...
				if ( ! empty( $product_variations ) ) {
                    ...
                    // phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared -- IDs are sanitized via intval() above.
					$products = $wpdb->get_results(
						"
					SELECT posts.post_parent
					FROM $wpdb->posts AS posts
					WHERE posts.ID IN (" . esc_sql( $product_variations_ids_string ) . ') AND posts.post_parent > 0',
						ARRAY_N
					);
					// +++
					$product_ids = array();
	...
	public function get_all_ids( $search_text, $options = array() ) {
		...
		// Allow external search engines (e.g. SearchWP) to override ID results
		$external_ids = apply_filters( 'woof_text_search_ids', null, $search_text, $options );
	...
	public function ajax_search() {
		...
		if ( WOOF_REQUEST::isset( 'link' ) ) {
			...
			$_GET        = apply_filters( 'woof_draw_products_get_args', map_deep( wp_unslash( $_GET ), 'sanitize_text_field' ), WOOF_REQUEST::get( 'link' ) );
		}
