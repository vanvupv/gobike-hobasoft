				...
				if ( ! empty( $prod_attributes ) ) {
					...
					if ( ! empty( $prod_attributes_in_request ) ) {
						...
						if ( $new_db ) {
                            ...
                            // phpcs:disable WordPress.DB.PreparedSQL.NotPrepared -- $attr_sql values are built with esc_sql(), not user input.
							$parent_ids = $wpdb->get_col(
								'SELECT a.product_or_parent_id '
                            ...
                            // phpcs:disable WordPress.DB.PreparedSQL.NotPrepared -- $attr_sql2 values are built with esc_sql(), not user input.
							$instock_parent_ids = $wpdb->get_col(
								'SELECT a.product_or_parent_id '
									. 'FROM ' . $wpdb->prefix . 'wc_product_attributes_lookup a '
									. 'JOIN  ' . $wpdb->prefix . 'wc_product_attributes_lookup b on b.product_id=a.product_id '
									. 'WHERE a.is_variation_attribute = 1'
							...
							foreach ( $prod_attributes_in_request as $attr_slug ) {
								...
								for ( $i = 0; $i < count( $terms ); $i++ ) {
									...
									if ( isset( $term_in_cycle[ $terms[ $i ] ] ) ) {
										...
										$t_name = $term_in_cycle[ $terms[ $i ] ] = $wpdb->get_var( $wpdb->prepare( "SELECT name FROM $wpdb->terms WHERE slug =%s", $terms[ $i ] ) );
									}
							...
							$args = array(
								'nopaging'         => true,
								// Required to bypass conflicting filters in this query context
								'suppress_filters' => true, // phpcs:ignore WordPressVIPMinimum.Performance.WPQueryParams.SuppressFilte
								'post_status'      => 'any',
