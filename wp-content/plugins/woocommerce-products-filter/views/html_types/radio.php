...
if ( ! function_exists( 'woof_draw_radio_childs' ) ) {
	...
	function woof_draw_radio_childs( $taxonomy_info, $tax_slug, $term_id, $childs, $show_count, $show_count_dynamic, $hide_dynamic_empty_pos, $parent_is_selected ) {
		...
		<?php if ( ! empty( $childs ) ) : ?>
			...
			<?php
			if ( $hide_childs == 1 and ! $parent_is_selected ) :
				?>
			...
			foreach ( $childs as $term ) :
				...
				?>
					...
					if ( woof()->settings['dispay_in_row'][ $tax_slug ] and empty( $term['childs'] ) ) :
						?>
						style="display: inline-block !important;"<?php endif; ?>>
						...
						<label <?php do_action( 'woof_print_label_attributes', $term ); ?>  class="woof_radio_label woof_radio_label_<?php echo esc_attr( $term['slug'] ); ?>
						<?php
						if ( in_array( $term['slug'], $current_request ) ) :
							?>
							woof_radio_label_selected<?php endif; ?>" for="<?php echo esc_attr( 'woof_' . $term['term_id'] . '_' . $inique_id ); ?>">
						<?php
					...
					<?php echo wp_kses_post( wp_unslash( $count_string ) ); ?></label>
						<a rel="nofollow" href="#" data-name="<?php echo esc_attr( woof()->check_slug( $tax_slug ) ); ?>" data-term-id="<?php echo esc_attr( $term['term_id'] ); ?>" style="
						<?php
						if ( ! in_array( $term['slug'], $current_request ) ) :
							?>
							...
							if ( in_array( $term['slug'], $current_request ) ) :
								?>
							woof_radio_term_reset_visible<?php endif; ?> woof_radio_term_reset_<?php echo esc_attr( $term['term_id'] ); ?>">
		...
		foreach ( $terms as $term ) :
			...
			if ( $hide_next_term_li ) :
				...
				<input type="radio" <?php if ( ! $count and ! in_array( $term['slug'], $current_request ) and $show_count ) : ?>
						...
						value="<?php echo esc_attr( $term['term_id'] ); ?>" <?php checked( in_array( $term['slug'], $current_request ) ); ?> />
				<label <?php do_action( 'woof_print_label_attributes', $term ); ?>  class="woof_radio_label 
				<?php
				if ( in_array( $term['slug'], $current_request ) ) :
					?>
					woof_radio_label_selected<?php endif; ?>" for="<?php echo esc_attr( 'woof_' . $term['term_id'] . '_' . $inique_id ); ?>">
			...
			<?php echo wp_kses_post( wp_unslash( $count_string ) ); ?></label>
				...
				<a rel="nofollow" href="#" data-name="<?php echo esc_attr( $this->check_slug( $tax_slug ) ); ?>" data-term-id="<?php echo esc_attr( $term['term_id'] ); ?>" style="
				<?php
				if ( ! in_array( $term['slug'], $current_request ) ) :
					?>
					display: none;<?php endif; ?>" class="woof_radio_term_reset  
