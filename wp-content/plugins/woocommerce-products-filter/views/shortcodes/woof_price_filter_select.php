...
<div class="woof_price_filter_dropdown_container">
	...
	<select id="<?php echo esc_attr( $select_id ); ?>" class="woof_price_filter_dropdown">
		<option value="-1"><?php echo esc_html( $price_filter2_1opt_txt ); ?></option>
		<?php if ( ! empty( $price2_filter_data ) ) : ?>
			...
			foreach ( $price2_filter_data['ranges']['options'] as $k => $value ) :
				...
				if ( $c == 0 and $show_count ) :
					?>
					disabled=""<?php endif; ?> <?php selected( $price2_filter_data['selected'], $k ); ?> value="<?php echo esc_attr( $k ); ?>"><?php echo wp_kses_post( wp_unslash( $value ) ); ?> <?php echo esc_html( $cs ); ?></option>
			<?php endforeach; ?>
