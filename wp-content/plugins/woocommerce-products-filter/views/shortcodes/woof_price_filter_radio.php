	...
	<div class="woof_container_inner">
		<ul class='woof_authors '>
			...
			if ( ! isset( $price2_filter_data['ranges']['options'] ) or ! is_array( $price2_filter_data['ranges']['options'] ) ) {
				...
				foreach ( $price2_filter_data['ranges']['options'] as $k => $value ) :
					$value = trim( $value );
					?>
					<?php
					$c  = 0;
					...
					<li class="woof_list">
						<input type="radio" 
						<?php
						if ( $c == 0 and $show_count ) :
							?>
							disabled=""<?php endif; ?> class="woof_price_filter_radio" <?php checked( $price2_filter_data['selected'], $k ); ?>  name="woof_price_radio" id="woof_price_radio_<?php echo esc_attr( $unique_id ); ?>" value="<?php echo esc_html( $k ); ?>"  />
						<span>&nbsp;&nbsp;</span><label for="woof_price_radio_<?php echo esc_attr( $unique_id ); ?>"><?php echo wp_kses_post( wp_unslash( $value ) ); ?> <?php echo esc_html( $cs ); ?>  </label>
						<a href="" data-tax="price" style="display: none;" class="woof_radio_price_reset 
						<?php
						if ( $price2_filter_data['selected'] == $k ) :
							?>
							woof_radio_term_reset_visible <?php endif; ?> woof_radio_term_reset_<?php echo esc_attr( $k ); ?>"><img src="<?php echo esc_url( $this->settings['delete_image'] ); ?>" height="12" width="12" alt="<?php esc_html_e( 'Delete', 'woocommerce-products-filter' ); ?>" /></a>
					</li>
					<?php
				endforeach;
			}
