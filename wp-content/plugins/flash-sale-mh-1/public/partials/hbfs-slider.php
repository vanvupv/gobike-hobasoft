...
$link_label    = ! empty( $json['link_label'] )   ? $json['link_label']   : __( 'Xem tất cả', 'hbweb-flashsale' );
$custom_css    = ! empty( $json['css'] )          ? $json['css']          : '';
$show_col      = ! empty( $json['show_col'] )     ? intval( $json['show_col'] )     : 5;
$product_limit = ! empty( $json['product_limit'] ) ? intval( $json['product_limit'] ) : 0;

// Autoplay settings — lấy từ admin, mặc định 3 giây
...
foreach ( $all_products as $p ) {
    ...
    if ( $product_limit <= 0 || count( $slot_products_map[ $s_idx ] ) < $product_limit ) {
...
<?php if ( $custom_css ) : ?>
<style>.hbfs-slider-wrap-<?php echo $slider_id; ?>{<?php echo strip_tags( $custom_css ); ?>}</style>
<?php endif; ?>

<div class="hbfs-slider-wrap hbfs-slider-wrap-<?php echo intval( $campaign['id'] ); ?>">

    <?php if ( $banner && empty( $json['persistent_banner'] ) ) : ?>
    ...
    <div class="hbfs-slider-box-frame" style="<?php echo $frame_style; ?>">
        ...
        <?php else : ?>
        <!-- Header Fallback -->
        <div class="hbfs-slider-header" style="<?php echo $style; ?>">
            <svg width="20" height="20" viewBox="0 0 24 24" fill="#fff"><path d="M7 2v11h3v9l7-12h-4l4-8z"/></svg>
            <span class="hbfs-slider-header__name"><?php echo esc_html( $campaign['name'] ); ?></span>

            <!-- Countdown -->
            <div class="hbfs-slider-header__countdown hbfs-countdown"
                 data-hbfs-countdown="<?php echo esc_attr( $time_end ); ?>">
            ...
            <a href="<?php echo esc_url( $link ); ?>" class="hbfs-slider-header__link">
                <?php echo esc_html( $link_label ); ?> &rsaquo;
            </a>
            <?php endif; ?>
        </div>
        <?php endif; ?>
    ...
    <div class="hbfs-products-row splide hbfs-slot-products-row hbfs-slot-row-<?php echo esc_attr( $s_idx ); ?>"
        ...
        if ( ! empty( $products ) ) :
            foreach ( $products as $p ) :
                ...
                $image        = ! empty( $p['post_thumbnail'] ) ? $p['post_thumbnail'] : ( ! empty( $p['image'] ) ? $p['image'] : wc_placeholder_img_src() );
                ...
                $sold          = intval( $p['sold'] );
                $qty           = intval( $p['qty'] );
                $discount      = ( $display_price && $regular && $regular > 0 )
                                 ? round( 100 - $display_price / $regular * 100 ) : 0;
                $sold_pct      = $qty > 0 ? min( 100, round( $sold / $qty * 100 ) ) : 0;
                ...
                $masked_price = hbfs_mask_price( $display_price );
        ?>
            ...
            <div class="item">
                <div class="news-item-products news-item-products-flashsale product-item" data-auto-slot="1">
                    ...
                    <div class="relative fix-images">
                        ...
                        <?php if ( $frame_img ) : ?>
                        <div class="hbfs-product-card__frame"><img src="<?php echo esc_url( $frame_img ); ?>" alt=""></div>
                        <?php endif; ?>
                    </div>
                    ...
                    <div class="price_flashsale">
                        <div class="price-content">
                            <?php if ( $is_slot_active ) : ?>
                                <div class="current-price" data-price="<?php echo esc_attr( $display_price ); ?>"><?php echo wc_price( $display_price ); ?></div>
                                <?php if ( $old_price && $old_price > $display_price ) : ?>
                                <div class="original-price"><?php echo wc_price( $old_price ); ?></div>
                                <?php endif; ?>
                            <?php else : ?>
                                <div class="current-price" data-price="<?php echo esc_attr( $display_price ); ?>"><?php echo esc_html( $masked_price ); ?></div>
                                <?php if ( $old_price ) : ?>
                                <div class="original-price"><?php echo wc_price( $old_price ); ?></div>
                                <?php endif; ?>
                            <?php endif; ?>
    ...
    </div><!-- .hbfs-products-row (Splide carousel) -->
    <?php endforeach; ?>

    </div><!-- .hbfs-slider-box-frame -->

</div><!-- .hbfs-slider-wrap -->
